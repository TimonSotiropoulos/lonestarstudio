...
class UploadImage extends Component {
    ...
    _renderImagePreview = () => {
        ...
        if (this.state.imageSelected) {
            if (this.props.type!=='circle'){
                return (
                    <C.Image image={this.state.previewImageURL} marginTop={S.Margin.Top_20} marginBottom={S.Margin.Bottom_20} width={'90%'} height={'500px'}/>
                );
            }else{
                return (
    ...
    render() {
        ...
        const { label, containerStyle, marginTop, marginBottom, marginLeft, marginRight, paddingTop, paddingBottom, paddingLeft, paddingRight, width, height } = this.props;
                ...
                {(label) ? <label htmlFor={inputKey} style={{cursor: 'pointer'}}><C.Text.Heading5 marginBottom={S.Margin.Bottom_20}>{label}</C.Text.Heading5></label> : null}
                    <Dropzone
                            ...
                            <div className={[S.Layout.flexRow, S.Layout.bothAxisCenter, S.Layout.flexWrap, S.Width._100, S.Margin.Bottom_20, S.Layout.textCenter].join(" ")}>
                                <C.Text.Link href="#" marginTop={S.Margin.Top_5} marginLeft={S.Margin.Left_5}>Upload image</C.Text.Link>
